        ...
        var handle_form_modal_btn_submit = function() {
            ...
            $('.validate-form').on('submit',function(){
                ...
                if( $("#pass1").val() != $("#pass2").val()  ){
                    var msg = "Las contraseñas ingresadas no son iguales";
                    $("#error").html( msg);
                 ...
                 */
                var tamanio = $("#pass1").val().length;
                console.log(tamanio);
                if(tamanio < 6 ){
                    var msg = "Su contraseña debe ser mayor o igual a <b>6</b> caracteres .";
                    $("#error").html( msg);
                    $("#error").removeClass('dis-none');
                    $('#bt_ingresar').prop('disabled', false);
                    check=false;
                ...
                if (check){
                    ...
                    $.post("/ingreso/cambia",{accion:"enviar",
                        "random":randomnumber,
                        "c": {/literal}'{$c}'{literal},
                        "e": {/literal}'{$e}'{literal},
                        "password":hex_md5($("#pass1").val())
                    }, function(data){
                        //console.log(data);
                        $("#cargando").addClass('dis-none');

                        if (data.resp == 1){
                            $("#div_pass1").addClass('dis-none');
                            $("#div_pass2").addClass('dis-none');
                            $("#error").addClass('dis-none');
                            $("#bt_enviar").hide();
