...
class Snippet extends Table
{
    ...
    public function get_item_datatable_Rows($item_id){
         ...
         */
        $extraWhere = " i.empresa_id = ".$this->empresa_id." and i.categoria_id=1";
        $groupBy = "";
        $having = "";
        /**
    ...
    function item_update($rec,$itemId,$que_form,$accion,$input_archivo){
         ...
         */
        $item = $objItem->get_item($this->empresa_id);
        $bandera = 0;
        if($item["estado_id"]==4){
            $res["res"] = 2;
         ...
         */
        if($accion == 'update') $item = $this->get_item($itemId);
        ...
        }else if(isset($item['categoria_id']) and $item['categoria_id']!=1){
            $res['res'] = 2;
        ...
        switch($que_form){
            case 'archivo':
                ...
                if ($accion=="new"){
                    ...
                    $dato_resultado["categoria_id"] = 1; //Empresa
    ...
    function get_item($id,$all=0){

        if($all == 0){
            $sql = "select p.*, e.observado from ".$this->tabla["empresa_pago"]." as p, empresa AS e where p.empresa_id = e.itemId AND  p.itemId = '".$id."' 
        and empresa_id = '" . $this->empresa_id. "' and categoria_id= '1'";
        }else{
            $sql = "select p.*, e.observado from ".$this->tabla["empresa_pago"]." as p, empresa AS e where p.empresa_id = e.itemId AND p.itemId = '".$id."' 
        and empresa_id = '" . $this->empresa_id. "' ";
        }
        ...
        if( ($item['itemId'] != "" and  $item['categoria_id']==1)
    ...
    function item_delete($id){
        ...
        if($item["itemId"]!="" and $item["modifica"]==1){
             ...
             */
            $carpeta = "pagos";
            $this->item_delete_archivo_sbm($id,$this->empresa_id,$carpeta);
            /**
    ...
    function item_enviar($id){
        $item = $this->get_item($id);

        if($item["itemId"]!="" and $item["modifica"]==1 and trim($item["adjunto_nombre"])!="" ){
            /**
             * Luego borrarmos el registro de la base de datos
             */
            $rec = array();
            $rec["estado_id"] = 2; //estado Enviado
            $where = "empresa_id = '".$this->empresa_id."' and itemId='".$id."'";
            $this->dbm->AutoExecute($this->tabla["empresa_pago"],$rec,"UPDATE",$where);
            $res['res'] = 1;
            $res['msg'] = "Pago enviado";
        }elseif (isset($item["adjunto_nombre"]) and $item["adjunto_nombre"] ==""){
            $res['res'] = 2;
            $res['msg'] = "El archivo de pago no existe";
        }else{
            $res['res'] = 2;
            $res['msg'] = "No es posible enviar el pago";
        }
        return $res;
    }
