...
class Snippet extends Table
{
    ...
    public function get_requisito_list($tipo_id, $item_id){
        ...
        $sql = "SELECT 
                    ...
                    WHERE cr.vicuna_tipo_id = '".$tipo_id."'
                    AND r.categoria_id = 4 ";
    ...
    function get_item_detalle($item_id){
        ...
        $sql = "select * from ".$this->tabla["vicuna"]." where itemId= '".$item_id."'  and empresa_id= '".$this->empresa_id."' ";
        $info = $this->dbm->Execute($sql);
        $info = $info->fields;
        return $info;
    }
    ...
    function get_item($id,$item_id){
        ...
        $sql = "SELECT 
                    ...
                    AND a.empresa_id = '".$this->empresa_id."'
                    WHERE cr.vicuna_tipo_id = '".$tipo_id."'
                    AND r.categoria_id = 4 AND a.itemId = '".$id."' and vicuna_id='".$item_id."' ";
    ...
    function item_update($rec,$itemId,$que_form,$accion,$item_id,$input_archivo){
       // print_struc($rec); exit();
        ...
        /**
         * Verificamos primero el estado de la archivo antes de guardar todos los datos
        ...
        if($item["estado_id"]==4){
            $res["res"] = 2;
            $res["msg"] = "Este archivo ya se encuentra Validado";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==2){
            $res["res"] = 2;
            $res["msg"] = "Este archivo esta en proceso de validación";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==5 or $item["estado_id"]==6 ){
            $res["res"] = 2;
            $res["msg"] = "Este archivo fue rechazado o cancelado";
            $res['accion'] = $accion;
        }else if($item["itemId"]!=""){
            $bandera = 1;
        }else{
        ...
        if($bandera==1){
            ...
            if($item["estado_id"]==4){
                 ...
                 */
                $campo_id="itemId";
                $where = "vicuna_id='".$item_id."' and empresa_id = ".$this->empresa_id ;
                ...
                if( $res["res"]==1){
                    ...
                    $adjunto = $this->item_adjunto_sbm($input_archivo,$item_id,$res["id"],$item,$tabla,$accion,"vicuna_id","requisitos");
                }
            }else{
                $res["res"] = 2;
                $res["msg"] = "No existe los datos que quiere modificar";
    ...
    function item_update_detalle($rec,$item_id,$que_form){
        ...
        if($item["estado_id"]==4){
            ...
            $res = $this->item_update_sbm($itemId,$respuesta_procesa,$this->tabla["vicuna"],$accion,$campo_id,$where);
            $res["accion"] = $accion;
    ...
    function item_delete($id,$item_id){
        ...
        if($bandera==1){
            ...
            if($item["estado_id"]==4){
                ...
                $tabla = $this->tabla["archivo"];
                $where = "empresa_id = '".$this->empresa_id."' and vicuna_id='".$item_id."' and itemId='".$id."' ";
