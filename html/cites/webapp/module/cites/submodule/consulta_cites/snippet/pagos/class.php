...
class Snippet extends Table
{
    ...
    function item_update($rec,$itemId,$que_form,$accion,$item_id,$input_archivo){
        ...
        if($bandera==1){
            ...
            if(($item["itemId"]!="" and $item["modifica"]==1)  or $accion=="new"){
                ...
                $campo_id="itemId";
                $where = " empresa_id = '".$this->empresa_id."' and cites_id= '".$item_id."' and categoria_id=2  " ;
            ...
            }else if(isset($item['categoria_id']) and $item['categoria_id']!=2){
                $res['res'] = 2;
                $res['msg'] = "El tipo de pago no se puede modificar.";
                $res['accion'] = $accion;
        ...
        switch($que_form){
            case 'archivo':
                ...
                if ($accion=="new"){
                    ...
                    $dato_resultado["categoria_id"] = 2; //CITES
                    $dato_resultado["estado_id"] = 1; //Registrado
                    //$dato_resultado["verificado"] = 0; //Registrado
    ...
    function item_enviar($id,$item_id){
        $item = $this->get_item($id,$item_id);
        ...
        if($item["itemId"]!="" and $item["modifica"]==1 and trim($item["adjunto_nombre"])!="" ){
            ...
            $where = "empresa_id = '".$this->empresa_id."' and itemId='".$id."' and cites_id='".$item_id."' ";
