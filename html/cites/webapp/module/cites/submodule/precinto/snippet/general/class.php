...
class Snippet extends Table
{
    ...
    function item_update($rec,$itemId,$que_form,$accion){
        ...
        if($item["estado_id"]==4){
            $res["res"] = 2;
            $res["msg"] = "Este precintado ya se encuentra Validado";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==2){
            $res["res"] = 2;
            $res["msg"] = "Este precintado esta en proceso de validación";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==5 or $item["estado_id"]==6 ){
            $res["res"] = 2;
            $res["msg"] = "Este precintado fue rechazado o cancelado";
            $res['accion'] = $accion;
