...
class Snippet extends Table
{
    ...
    public function get_item_datatable_Rows($item_id){
         ...
         */
        $extraWhere = " i.empresa_id = ".$this->empresa_id." and i.dispensacion_id='".$item_id."' ";
    ...
    function item_update($rec,$itemId,$que_form,$accion,$item_id){
        global $objItem;
        /**
         * Verificamos primero el estado de dispensacion antes de guardar todos los datos
        ...
        if($item["estado_id"]==4){
            $res["res"] = 2;
            $res["msg"] = "Esta dispensacion ya se encuentra Validado";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==2){
            $res["res"] = 2;
            $res["msg"] = "Esta dispensacion esta en proceso de validación";
            $res['accion'] = $accion;
        }else if($item["estado_id"]==5 or $item["estado_id"]==6 ){
            $res["res"] = 2;
            $res["msg"] = "Esta dispensacion fue rechazado o cancelado";
            $res['accion'] = $accion;
        }else if($item["itemId"]!=""){
            $bandera = 1;
        }else{
        ...
        if($bandera==1){
            ...
            if(($item["itemId"]!="")  or $accion=="new"){
                ...
                $campo_id="itemId";
                $where = " empresa_id = '".$this->empresa_id."' and dispensacion_id= '".$item_id."' " ;
    ...
    function item_delete($id,$item_id){
        global $objItem;
        /**
         * Verificamos primero el estado de a dispensacion antes de guardar todos los datos
         * el estado del padre es el que manda al momento de guardar los datos
         */
        $item = $objItem->get_item($item_id);
        $bandera = 0;
