    ...
    var snippet_resumen = function () {
        var resumen = function (){
            ...
            $.get( url, {},
                function(res){
                    ...
                    $("#nombre_importador").html(res.importador_nombre);
                    $("#nombre_exportador").html(res.exportador_nombre);
                    $("#direccion_exportador").html(res.exportador_direccion);
                    $("#direccion_importador").html(res.importador_direccion);
                    $("#nombre_exportador").html(res.exportador_nombre);
                    $("#direccion_exportador").html(res.exportador_direccion);
                    $("#pais_exportador").html(res.paisexportador);
                    $("#pais_importador").html(res.paisimportador);
                    $("#tipo_documento").html(res.documento);
                    $("#proposito").html(res.proposito);*/
                },"json");
        };
        ...
        var snippet_item = function () {
            ...
            var enviar = function (){
                ...
                swal({
                    title: '¿Esta seguro de Enviar la solicitud de Dispensación?',
                    html: 'Recuerde que una vez enviada la solicitud  <br>'+
                        '<span style="color:red;"><strong>NO PODRA REALIZAR NINGUNA MODIFICACIÓN </strong></span>',
                    type: 'warning',
            ...
            var enviar_accion = function () {
                ...
                $.get( url,{},
                    function(res){
                        ...
                        if(res.res == 1){
                            swal.close();
                            swal({position: 'top-center',type: 'success',title: 'La solicitud fue enviada',showConfirmButton: false,timer: 1000});
