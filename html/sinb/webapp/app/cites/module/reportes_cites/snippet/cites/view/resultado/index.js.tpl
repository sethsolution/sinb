...
<script>
    ...
    var ctx_monto_dos = document.getElementById("chart_estado").getContext('2d');
    var chart_monto_dos = new Chart(ctx_monto_dos, {
        ...
        data: {
            labels: [
                ...
                {if $idx != 0},{/if}"{$row.year}"
                {/foreach}
                {literal}
            ],
            datasets: [
                {/literal}
                {foreach from=$res.estadoCites.citesEstado item=row key=idx}
                {if $idx != 0},{/if}
                {
                    label: '{$row.estado}',
                    data: [
                        {foreach from=$row.year item=dato key=idx2}
