...
<script>
    ...
    var ctx_monto_dos = document.getElementById("chart_especie").getContext('2d');
    var chart_monto_dos = new Chart(ctx_monto_dos, {
        ...
        data: {
            labels: [
                ...
                {if $idx != 0},{/if}"{$row.especie}"
                {/foreach}
                {literal}
            ],
            datasets: [
                {/literal}
                {foreach from=$res.mesEspecie.meses item=row key=idx}
                {if $idx != 0},{/if}
                {
                    label: '{$row.nombre}',
                    data: [
                        {foreach from=$row.especie item=dato key=idx2}
