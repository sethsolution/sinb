<?PHP
namespace App\Icas\Module\Report\Snippet\Familia;
...
class Index extends CoreResources
{
    ...
    function getEstadoTotal($estado_id,$tipo_id){
        $sql = "select p.estado_id, sum(p.familia_beneficiarias_total) as total
                from proyecto as p
    ...
    function consulta($item){
        ...
        $sql = "select ct.id ,ct.nombre as tipo,re.total
                from
                (
                select p.tipo_id ,sum(p.familia_beneficiarias_total) as total
                from proyecto as p 
                ".$where."
                GROUP BY p.tipo_id
                ) as re
        ...
        $sql = "select ce.id, ce.nombre as estado ,t.*
                    from(
                    select p.estado_id, sum(p.familia_beneficiarias_total) as total
                    from proyecto as p
                    ".$where."
                    GROUP BY p.estado_id
                    ) as t
        ...
        $sql = "select re.tipo_id, ct.nombre as tipo, ce.nombre as estado, re.total
                from
                (
                select p.tipo_id, p.estado_id, sum(p.familia_beneficiarias_total) as total
                from proyecto as p
                ".$where."
    ...
    public function getTipoYear($year,$id,$where,$join){
        ...
        $sql = "select sum(p.familia_beneficiarias_total) as total
                from proyecto as p
                ".$where." ";
    ...
    public function getDepartamento($filtro){
        ...
        $sql = "select 
            ...
            (
            SELECT  
            p.departamento_id,sum(p.familia_beneficiarias_total) as total
            FROM proyecto as p
            ".$where."
    ...
    public function getProyectoTipo($filtro,$years,$tipo,$estados){
        ...
        $sql = "select 
                ...
                (
                select 
                p.tipo_proyecto_id, sum(p.familia_beneficiarias_total) as total
                from proyecto as p
                ".$where."
