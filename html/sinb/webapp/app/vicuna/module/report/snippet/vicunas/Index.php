...
class Index extends CoreResources
{
    ...
    function consulta($item){
        ...
        foreach ($departamento as $row){
            ...
            $totalMachos = $totalMachos + (int)$row["total_machos"];
            $totalHembras = $totalHembras + (int)$row["total_hembras"];
        }
        $res["total"] = $total;
        $res["totalHembras"] = $totalHembras;
        $res["totalMachos"] = $totalMachos;

        /**
         * Sacamos los datos para la tabla
         */
        $sql = "select re.departamento_id, d.name as departamento, m.name as municipio, re.total, re.total_hembras, re.total_machos
                from
                (
                select i.departamento_id, i.municipio_id, sum(i.numero_vicuna_esquiladas) as total,
                       sum(i.captura_machos) as total_machos, sum(i.captura_hembras) as total_hembras
    ...
    public function getDepartamento($filtro){
        ...
        $sql = "select 
            ...
            , dep.total, dep.total_hembras, dep.total_machos
            from
            (
            SELECT  
            i.departamento_id, sum(i.numero_vicuna_esquiladas) as total,  
                   sum(i.captura_machos) as total_machos, sum(i.captura_hembras) as total_hembras
            FROM vicuna.esquila as i
            ".$where."
