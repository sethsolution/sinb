<?PHP
namespace App\Ccfs\Module\Ccfs_listado\Snippet\Ubicacion;
...
class Index extends CoreResources
{
    ...
    function updateData($rec,$itemId,$form,$action,$item_id=""){
        ...
        switch($form){
            case 'index': $tabla = $this->table[$this->objTable];
                break;
            case 'module': $tabla = $this->table["ccfs"];
                break;
        }
    ...
    function getTotalUbicaion($id,$main=""){
        ...
        $sql = "select * from ccfs as pu where ".$where." pu.id = ".$id;
        $item = $this->dbm->Execute($sql);
        $item = $item->GetRows();
        return $item;
    }
    ...
    public function setMain($id,$item_id){
        if($id>0 and $item_id>0){
            $sql = "update ccfs as pu set main=FALSE where pu.id =".$item_id;
            $this->dbm->execute($sql);

            $sql = "update ccfs as pu set main=TRUE where pu.id = ".$id;
            $this->dbm->execute($sql);
