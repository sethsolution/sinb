...
class SSP {
    ...
    static function filter ( $request, $columns, &$bindings, $isJoin = false )
    {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    ...
                    $globalSearch[] = ($isJoin) ? $column['db']." iLIKE ".$binding : " ".$column['db']." iLIKE ".$binding;
                }
            }
        }
        ...
        for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
            ...
            if ( $requestColumn['searchable'] == 'true' &&
                $str != '' ) {
                $binding = SSP::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                $columnSearch[] = ($isJoin) ? $column['db']." iLIKE ".$binding : " ".$column['db']." iLIKE ".$binding;
            }
        }
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT  ".implode(", ", $col)."
			 ...
			 $order
			 $limit";
            $query_found_rows =  "SELECT COUNT(i.{$primaryKey})
			 $joinQuery $where $extraWhere $groupBy $having ";
        }else{
            $query =  "SELECT  ".implode(", ", SSP::pluck($columns, 'db'))."
			 FROM $table
			 $where $extraWhere $groupBy $having $order $limit";

            $query_found_rows =  "SELECT COUNT(i.{$primaryKey})
			 FROM $table
			 $where $extraWhere $groupBy $having ";
        }
        ...
        $resFilterLength = SSP::sql_exec( $db,$bindings,$query_found_rows);
    ...
    {
        try {
            $dsn  = $sql_details["type"].':host='.$sql_details["host"].';dbname='.$sql_details["db"].';port='.$sql_details["port"] ;
            $db = @new PDO($dsn,
                $sql_details["user"],
                $sql_details["pass"],
