...
class SSP {
        ...
        for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
            ...
            for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
                ...
                if ( isset( $column['formatter'] ) ) {
                    ...
                    if(isset( $column['as'])){
                        $row[ $column['dt'] ] = ($isJoin) ? $data[$i][ $columns[$j]['dt'] ] : $data[$i][ $columns[$j]['db'] ];
                    }else{
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        if($joinQuery){
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS ".implode("\n, ", $col)."
			 $joinQuery
			 $where
			 $extraWhere
			 $groupBy
    ...
    static function data_output_pg($columns, $data, $isJoin = false) {
        $out = array();
     ...
     *  @return string SQL limit clause
     */
    static function limit_pg($request, $columns) {
        $limit = '';
     ...
     *  @return string SQL order by clause
     */
    static function order_pg($request, $columns, $isJoin = false) {
        $order = '';

        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = SSP::pluck_pg( $columns, 'dt' );
     ...
     *  @return string SQL where clause
     */
    static function filter_pg($request, $columns, &$bindings, $isJoin = false) {
        $globalSearch = array();
        $columnSearch = array();
        $dtColumns = SSP::pluck_pg( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = SSP::bind_pg( $bindings, '%'.$str.'%', PDO::PARAM_STR );
                    //$globalSearch[] = ($isJoin) ? $column['db']." LIKE ".$binding : "`".$column['db']."` LIKE ".$binding;
     ...
     *
     */
    static function simple_pg($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '') {
        global $datatable_debug;
        $bindings = array();
        $db = SSP::sql_connect_pg( $sql_details );
        // Build the SQL query string from the request
        $limit = SSP::limit_pg( $request, $columns );
        $order = SSP::order_pg( $request, $columns, $joinQuery );
        $where = SSP::filter_pg( $request, $columns, $bindings, $joinQuery );
        ...
        // Main query to actually get the data
        if($joinQuery){
            $col = SSP::pluck_pg($columns, 'db', $joinQuery);
            /*$query =  "SELECT SQL_CALC_FOUND_ROWS ".implode(", ", $col)."
             ...
             $order
             $limit";*/
             $query =  "SELECT ".implode(", ", $col).", count(*) OVER() AS total_count 
             $joinQuery
             $where
             $extraWhere
             $groupBy
            ...
            /*$query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSP::pluck($columns, 'db'))."`
             ...
             $order
             $limit";*/
             $query =  "SELECT ".implode(", ", SSP::pluck_pg($columns, 'db')).", count(*) OVER() AS total_count 
             FROM $table
             $where
             $extraWhere
             $groupBy
        ...
        */
        //echo $query."  <br>"; exit;
        $data = SSP::sql_exec_pg( $db, $bindings,$query);
        //print_struc($data);exit;
        // Data set length after filtering
        /*$resFilterLength = SSP::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );*/
        $resFilterLength = $data[0]["total_count"];
        ...
        $resTotalLength = SSP::sql_exec_pg( $db,
            "SELECT COUNT({$primaryKey})
             FROM   $table "
        );
        $recordsTotal = $resTotalLength[0][0];
        ...
        return array(
            ...
            "recordsTotal"    => intval( $recordsTotal ),
            "recordsFiltered" => intval( $recordsFiltered ),
            "data"            => SSP::data_output_pg( $columns, $data, $joinQuery )
        );
    }
     ...
     * @return resource Database connection handle
     */
    static function sql_connect_pg($sql_details) {
        try {
            $db = @new PDO(
     ...
     * @return array         Result from the query (all rows)
     */
    static function sql_exec_pg($db, $bindings, $sql=null) {
        // Argument shifting
        if ( $sql === null ) {
            $sql = $bindings;
        }
        ...
        */
        ...
        try {
            ...
            SSP::fatal_pg( "An SQL error occurred: ".$e->getMessage() );
        }
     ...
     *   would be used.
     */
    static function bind_pg(&$a, $val, $type) {
        $key = ':binding_'.count( $a );
     ...
     *  @return array        Array of property values
     */
    static function pluck_pg($a, $prop, $isJoin = false) {
        $out = array();
