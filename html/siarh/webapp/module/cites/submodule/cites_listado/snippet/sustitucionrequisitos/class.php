...
class Snippet extends Table
{
    ...
    function get_item_id($item_id){
        $sql = "select * from ".$this->tabla["sustitucion"]." where cites_id_sustitucion= '".$item_id."'  ";
        $info = $this->dbm->Execute($sql);
        $info = $info->fields;
        return $info;
    }
    ...
    function get_file($id,$item_id,$tipo){
        ...
        if($item["itemId"]!=""){
            $dir  = $this->get_dir_item_archivo_sbm($item["sustitucion_id"],0,"requisitos");
    ...
    function item_update_detalle($rec,$item_id,$que_form){
        ...
        if($item["estado_id"]==4) {
            ...
            $where = "";
            //$this->dbm->debug=true;
            $res = $this->item_update_sbm($sus_detalle["itemId"],$respuesta_procesa,$this->tabla["sustitucion"],$accion,$campo_id,$where);
            $res["accion"] = $accion;
