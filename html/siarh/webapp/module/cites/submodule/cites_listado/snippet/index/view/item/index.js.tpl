...
<script>
        ...
        var resumen = function (){
            ...
            $.get( url, {},
                function(res){
                    ...
                    /**
                     * Verificamos la aprobacion de items
                     */
                    requisitos = res.archivos_aprobados;
                    pagos = res.pagos_aprobados;
                    especies = res.especies_aprobadas;
                    /**
    ...
    var snippet_item = function () {
        ...
        var btn_imprime_excel = $('#btn_imprime_excel');
        var btn_ver = $('#btn_ver');
        ...
        var aprobar_accion = function () {
            ...
            $.get( url,{
                ...
                function(res){
                    ...
                    if(res.res == 1){
                        swal.close();
                        swal({position: 'top-center',type: 'success',title: 'La solicitud de certificación CITES fue aprobada<br> Se envió un correo electronico al usuario. ',showConfirmButton: false,timer: 2000});
        ...
        var aprobar = function (){
            ...
            console.log(pagos);
            if (requisitos > 0){
                html_msg += '<span style="color:#bf8516;"> - Se recomienda la revisión y aprobación de <strong>requisitos</strong></span> <br>';
            }else{
                html_msg += '';
            }
            if (pagos > 0){
                html_msg += '<span style="color:#bf8516;">- Se recomienda la revisión y aprobación de la <strong>Boleta de depósito</strong></span><br>';
            }else{
                html_msg += '';
            }
            if (especies > 0){
                html_msg += '<span style="color:#bf8516;">- Se recomienda la revisión y aprobación de <strong>Especies</strong></span><br>';
            }else{
                html_msg += '';
            }
            html_msg += '<br> Recuerde que una vez aprobada la solicitud, se enviara un email de aprobación al usuario y  <br>'+
        ...
        var boton_imprime = function () {
            btn_imprime.click(function(e) {
                ...
                console.log (url);
                window.open(url,'impresion_cites_preview');
            });
        };

        var boton_imprime_excel = function () {
            btn_imprime_excel.click(function(e) {
                e.preventDefault();
                randomnumber=Math.floor(Math.random()*11);
                url = "{/literal}{$getModule}&accion=imprime.excel&rand="+randomnumber+"&item_id={$id}{literal}";
                //console.log (url);
                window.open(url,'impresion_cites_excel');
                //alert("imprime excel");
            });
        };
