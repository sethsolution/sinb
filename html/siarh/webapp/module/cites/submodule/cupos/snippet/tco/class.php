...
class Snippet extends Table
{
    ...
    function procesa_datos($que_form,$rec,$accion="new",$item_id){
        ...
        switch($que_form){
            case 'archivo':
                ...
                if($dato_resultado["tco_id"]!=""){
                    $sql = "select nombre from catalogo_tco as t where t.itemId='".$dato_resultado["tco_id"]."' ";
                    $tco = $this->dbm->execute($sql);
                    $tco = $tco->fields;
                    $dato_resultado["tco"] = $tco["nombre"];
                }
    ...
    function get_item($id,$item_id){
        $sql = "select * from ".$this->tabla["tco"]." as p where p.itemId = '".$id."' and gestion_id='".$item_id."'";
    ...
    function item_delete($id,$item_id){
        ...
        if($item["itemId"]!=""){
            ...
            $campo_id="itemId";
            $where = "gestion_id='".$item_id."' ";
            $res = $this->item_delete_sbm($id,$campo_id,$this->tabla["tco"],$where);
        }else{
            $res['res'] = 2;
