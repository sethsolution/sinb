...
class Index extends Table {
    ...
    function get_item_destalle($idItem){
     ...
     function get_item_persona(){
        $sql = " select * from ".$this->tabla["persona"]." as u where  u.itemId = '".$this->userv["persona_id"]."'";
    ...
    function get_item1(){
      //print_struc($this->userv["itemId"]);
        $sql = "select * from ".$this->tabla["persona"]." as p where p.itemId = '".$this->userv["persona_id"]."'";
        $item = $this->dbm->Execute($sql);
        $item = $item->fields;
        return $item;
    }
     ...
     $sql ="select  count(p.itemId) as conteo 
              from ".$this->tabla["persona"]." as p
              left join ".$this->tabla["persona_formacion_academica"]." as d on p.itemId=d.persona_id
              where p.itemId = '".$id."' and d.verificado=0";
        $info = $this->dbm->Execute($sql);
        $info = $info->fields;
        return $info;
    }
