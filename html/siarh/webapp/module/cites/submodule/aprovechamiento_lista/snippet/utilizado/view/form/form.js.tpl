...
<script>
    var snippet_form_modal = function() {
        ...
        var showResponse = function (responseText, statusText) {
            ...
            if(responseText.res ==1){
                ...
                if(responseText.accion =='update') {
                    swal({type: 'success',title: 'Se guardo la información con exito!',showConfirmButton: false,timer: 2000});
                }else{
                    swal({type: 'success',title: 'Se adicionó la ESPECIE a transitar, con exito!!!',showConfirmButton: false,timer: 2000});
                }
            }else{
        ...
        var handle_form_modal_components = function(){
            ...
            $('.calcula').keyup(function(evt, params){
                calculadora();
            });
            ...
            var calculadora = function (){
                /**
                 * Saca la cantidad de unidades
                 */
                inicio = $('#inicio').val();
                if(inicio == "") inicio = 0;
                /**
                 * Saca el costo por unidad
                 */
                fin = $('#fin').val();
                if(fin == "") fin = 0;
        ...
        var msg_error = function(){
         ...
         var handle_select_activo = function() {
             $('#select_origen').on('change', function () {
                 ...
                 var origen_div = $('#origen_div');
                 origen_div.addClass('m--hide');
                 switch (id) {
                     case '7':
                         origen_div.removeClass('m--hide');
                         break;
                 }
             });
             ...
             $('#select_unidad').on('change', function () {
                 var id = $(this).val();
                 id1 = id == null ? '' : id.toString();
                 var unidad_div = $('#unidad_div');
                 unidad_div.addClass('m--hide')
                 switch (id) {
                     case '27':
                         unidad_div.removeClass('m--hide');
                         break;
                 }
             });
