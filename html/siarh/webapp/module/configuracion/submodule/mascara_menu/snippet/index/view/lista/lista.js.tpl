                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if(full.estado2 != 3 && full.estado2 != 4){
                                boton += '<a href="javascript:item_update(\''+data+'\');" class="m--margin-right-5 btn btn-success btn-sm m--margin-bottom-5" title="Modificar"><i class="fab fa-gg"></i>&nbsp;Modificar</a>';
                            }
                            {/literal}{/if}{literal}

                            {/literal}{if $privFace.eliminar == 1}{literal}
                            if(full.estado2 != 3){
                                boton += '<a href="javascript:item_delete(\''+data+'\');" class="btn btn-danger btn-sm m--margin-bottom-5" title="Eliminar"><i class="la flaticon-delete-2"></i>&nbsp;Eliminar</a>';
                            }
                            {/literal}{/if}{literal}
                            //'';
                            return boton;
...

                    {
                        ...
                        render: function(data,type,full,meta){
                            var status = {
                                'INDEX': {'title': 'SUBSISTEMA', 'state': 'brand'},
                                'URL': {'title': 'URL', 'state': 'brand'},
                                'SB': {'title': 'SUBMODULO DE SUBSISTEMA', 'state': 'brand'}
                            };
                            if (typeof status[data] === 'undefined') {
                                return data;
                            }
                            return'<span class="m--font-bold m--font-' + status[data].state + '">' + status[data].title + '</span>';
                        },
