...
class Snippet extends Table
{
        ...
        switch($que_form){
            case 'archivo':
                ...
                if ($accion=="new"){
                    ...
                    $dato_resultado["categoria_id"] = $item_id;

                }

                switch ($dato_resultado["tipo"]){
                    case 'URL':
                        unset($dato_resultado["submodulo_id"]);
                        unset($dato_resultado["modulo_id"]);
                        break;

                    case 'INDEX':
                        unset($dato_resultado["submodulo_id"]);
                        unset($dato_resultado["url"]);
                        break;
                    case 'SB':
                        unset($dato_resultado["url"]);
                        unset($dato_resultado["modulo_id"]);
    ...
    function get_item($id,$item_id){
        //$this->dbm->debug = true;
        $sql = "select * from ".$this->tabla["menu"]." as p where p.itemId = '".$id."' and p.categoria_id = '".$item_id."'";
    ...
    function item_delete($id,$item_id){
         ...
         */
        $campo_id="itemId";
        $where = "categoria_id='".$item_id."'";
        $res = $this->item_delete_sbm($id,$campo_id,$this->tabla["menu"],$where);
