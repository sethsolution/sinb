	...
	function __construct(){
       ...
       $this->dataItem = array("usuario"=>array(1=>array("campo"=>"portada"
                                            ...
                                            ,17=>array("campo"=>"dateUpdate"
                                                      ,"descripcion"=>"Fecha Actualización"))
			...
			for ( $i=0, $ien=count($arrayGet['columns']) ; $i<$ien ; $i++ ) {
				...
				if ( $requestColumn['searchable'] == 'true' ) { 
                  ...
                  if(isset($dataItem[$columnIdx-1]["tabla"])){
				    ...
				    if($columnIdx==6){
				      $globalSearch[] = " IF(i.tipoUsuario=0,'administrador',IF(i.tipoUsuario=1,'administrador','normal')) LIKE '%".$valFilter."%'";   
				    }else{
    	...
    	foreach ( $rResult as $aRow ){
          ...
          for ( $i=0 ; $i<count($columns) ; $i++ ){  
             if($i==0){
               ...
               $row[] = utf8_encode($listTipoUsuario[$aRow[$columns[$i]]]);
             }elseif($i==16){
