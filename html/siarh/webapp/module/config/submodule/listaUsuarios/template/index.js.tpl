...
function itemDelete(id,data){
    $.msgbox("Esta seguro de eliminar este registro?<br>Info<br><b>"+data+"</b>"
            ,   {type: "confirm",buttons : [{type: "submit", value: "Si"}
            ,   {type: "cancel", value: "No"}]}
            ,   function(result){
                    if(result){itemDeleteAction(id);}
            });
}
...
function setFilterTable(colFilter,valFilter){
    oSettingsItem.aoPreSearchCols[colFilter].sSearch = (valFilter==0 && colFilter!=16)? "": valFilter;
    item_oTable.draw();
}
