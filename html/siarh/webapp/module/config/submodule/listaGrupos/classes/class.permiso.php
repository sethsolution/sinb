...
class Permiso extends Table {
	...
	function __construct(){
       ...
       $this->tableName[0] = $smoduleTableName."core_grupo_permisos";
    ...
    /** 
     * Construye el json del datatable para listado de grupos
...
//	$dtColumns = $columns;
	...
	//	$dtColumns = $this->pluck( $columns, 'dt' );
		...
		$where = " i.grupoId=$fichaId and i.tipo=0 ";
...
//echo "=>".$sql;
    ...
    function listCatalogoPermisoUsuario($itemId){
        ...
        $sql = 'SELECT m.itemId, m.titulo
                ...
                WHERE i.grupoId='.$itemId." AND i.subModuloId=sm.itemId AND sm.moduloId=m.itemId 
                GROUP BY m.itemId
    ...
    function itemSave($rec,$tipoTabla=0){
      ...
      if($privFace["crear"]){
        ...
        if($resprocesa["res"]==1){
          ...
          $recrow["grupoId"] = $resprocesa["rec"]["id"];
      ...
      * @return $res ARRAY
      **/ 
    function verificarParent($submoduloId,$grupoId){
        global $dbm;
        $dbm->SetFetchMode(ADODB_FETCH_ASSOC);

        $sql = 'SELECT i.parent, u.subModuloId 
                FROM '.$this->tableName[2].' i
                LEFT JOIN '.$this->tableName[0].' u ON u.subModuloId=i.parent AND u.grupoId='.$grupoId.'
    ...
    function procesaDatos($tipoTabla,$rec,$accion="new"){
        ...
        switch($tipoTabla){
            case '0':
                    ...
                    if(count($rec["list"])==0){
                      $bRigth=1;
                      $msgRigth.=utf8_encode('<br>- Seleccione submodulos para el grupo.');  
                    }
                      
                    if(trim($rec["id"])==0 || trim($rec["id"])==""){
                      $bRigth=1;
                      $msgRigth.=utf8_encode('<br>- ID de grupo inv�lido.');  
                    }
      ...
      if(trim($itemId)!="" && trim($itemId)!=0){
        ...
        if(count($list)>0){
            ...
            foreach($list as $idx=>$row){
              ...
              $resupdate = $dbm->AutoExecute($this->tableName[0],$row,"UPDATE"," grupoId=".$itemId." AND subModuloId=".$idx);
        ...
        $res["res"]=2;
        $res["msg"] = utf8_encode("No existe el ID del grupo.");  
      }
     ...
     function deleteItem($id,$itemId){
        ...
        if($privFace["eliminar"]){
            ...
            if(trim($id)){
                ...
                $sql = "delete from ".$this->tableName[0]." WHERE subModuloId=".$id." AND grupoId=".$itemId;//echo ":::".$sql;
