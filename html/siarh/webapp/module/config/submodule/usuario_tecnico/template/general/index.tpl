...
<tr>
      ...
      <legend >Datos de usuario</legend>
        ...
        <tr>
            ...
            <td class="respuesta {if $idx == 2}lt{/if} ld">
                {if $row.tipoInput == 'text' || $row.tipoInput == 'number' || $row.tipoInput == 'decimal'}
                  <input DISABLED type="text" class="input" name="item[{$row.campo}]" 
                ...
                {elseif $row.tipoInput == 'select'}
                  <select disabled class="input itemFilter" name="item[{$row.campo}]" 
                  id="form_{$row.campo}" {$privFace.input} >
        ...
        <tr id="grupoShow">
            ...
            <td class="respuesta ld">
                <select  disabled style="width:97%;border-color:orange;" 
    ...
    <tr>
        ...
        <td class="respuesta lt ld">
            <select disabled class="itemFilter" name="item[institucionId]" 
            {$privFace.input} id="idInstitucion"
            data-placeholder="Elija una instituci�n">
            <option value="0">Seleccione una Instituci&oacute;n</option>
    ...
    <tr>
        ...
        <td class="respuesta ld" >
            <select disabled class="itemFilter" name="item[programas][]" 
                    ...
                    {if $listCatalogo.19|@count gt 0}
                      {foreach key=rowId item=row from=$listCatalogo.19}
                        <div style="width:auto;padding:4px 0px;display:inline-block;">
                          <input DISABLED type="radio" name="item[coordinador]" class="input ubic_opt" value="{$rowId}"
        ...
        {foreach from=$listDataPersonal item=row key=idx name=personal}
            <tr>
                ...
                <td class="respuesta {if $idx == 3}lt{/if} ld">
                    {if $row.tipoInput == 'text' || $row.tipoInput == 'number' || $row.tipoInput == 'decimal'}
                        <input DISABLED type="text" name="item[{$row.campo}]" 
                        id="form_{$row.campo}" {$privFace.input} 
                        ...
                        {/if} />
                    {elseif $row.tipoInput == 'select'}
                        <select disabled class="input" name="item[{$row.campo}]" id="form_{$row.campo}" {$privFace.input}>
                        {html_options options=$listCatalogo.$idx selected=$item[$row.campo]}
                        </select>
                    {/if}
                </td>
