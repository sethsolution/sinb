...
class Index extends Table {
    ...
    function get_menu_principal(){
        ...
        for ($i=0;$i<count($item);$i++){
            ...
            left join ".$this->tabla_core["submodulo"]." as sub on sub.itemId = m.submodulo_id
            left join ".$this->tabla_core["modulo"]." as mo2 on mo2.itemId= sub.moduloId
            where m.activo=1 and m.categoria_id = '".$item[$i]["itemId"]."' ";


            //print_struc($_SESSION["userv"]["tipoUsuario"]);exit;
            if($_SESSION["userv"]["tipoUsuario"]!=1){
                //print_struc($_SESSION["userv"]);
                //print_struc($this->tabla_core);

                $sql2 .= "
                AND 
				if(m.tipo = 'INDEX', 
				 (SELECT COUNT(*) AS total FROM ".$this->tabla_core["usuario_permisos"]." AS p 
				 LEFT JOIN ".$this->tabla_core["submodulo"]." AS sb ON sb.itemId = p.subModuloId
				  WHERE p.usuarioId='".$_SESSION["userv"]["itemId"]."' 
				  AND sb.moduloId =m.modulo_id
				  ) 
				 ...
				 if(m.tipo = 'SB',
				 	(
					 SELECT COUNT(*) AS total FROM ".$this->tabla_core["usuario_permisos"]." AS p
					 WHERE p.usuarioId = '".$_SESSION["userv"]["itemId"]."' AND  p.subModuloId = m.submodulo_id 
					 )
