...
class Index extends Table {
    ...
    function obtenerTco(){
        ...
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
                ...
                WHERE u2.itemId= ".$_SESSION["memberId"]." and (u.itemId=".$_SESSION["memberId"]." or ur2.rol_itemId=1 or ur2.rol_itemId=4 or ur2.rol_itemId=6)
    ...
    function obtenerFilasReporte($id_acta=""){
        ...
        for($i=1;$i<=8;$i++){
            ...
            $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
            $sql='SELECT  tccl.*,CONCAT_WS(" a ",lc.min,lc.max) as longitud_literal
            FROM  tco_custodia_cuero_longitud tccl
            INNER JOIN longitud_cuero lc on 
            lc.itemId=tccl.longitud_cuero_itemId
            WHERE tccl.tco_custodia_cuero_itemId='.$id_acta.'
            ORDER BY tccl.longitud_cuero_itemId'
            ;
            $info = $this->dbm->Execute($sql);
            $info = $info->GetRows();
    ...
    public function get_item_datatable_Rows($gestion=2019){
         ...
         */
        $ver=$this->verificar_permiso_usuarios(["1","3","6"]);
        if($ver){
            $tco=$this->obtenerTCO();
            if($tco){
                if($tco["rol_itemId"]=="1"||$tco["rol_itemId"]=="6"){
                    ...
                    $extraWhere .= " and tco.itemId=".$tco["tco_itemId"];
                    $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
                }
                $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
                }   
    ...
    function item_delete($id){
        $campo_id="itemId";
            $res = $this->item_soft_delete_sbm($id,$campo_id,"tco_custodia_carne");
        return $res;
    }
