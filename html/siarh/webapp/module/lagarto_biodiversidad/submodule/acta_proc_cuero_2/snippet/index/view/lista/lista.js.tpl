...
<script>
    ...
    function item_download(data){
        ...
        req.open("GET", direccionAPI+'acta-cuero/pdf/'+data, true);
        ...
        req.onload = function (event) {
            ...
            var fileName = "cuero-"+data+".pdf";//if you have the fileName header available
    ...
    var snippet_datatable_list = function () {
        ...
        var exporta_titulo = "Venta_Cuero_Responsable_Empresa";
        var exporta_columnas = [':visible :not(.noExport)' ];

        var initTable1 = function() {
            // begin first table
if(!{/literal}{json_encode($gestionVer)}{literal}){
                    swal({position: 'center',type: 'warning',title: 'No hay un registro para la Gestion actual. \n Si quiere Registrar la gestión actual vaya a "Administrar Gestión"',showConfirmButton: true});
}
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            var tipo="Verificar";
                            var href=''+
                                    {/literal}{if $usuarioInfo.rol_itemId == 1||$usuarioInfo.rol_itemId == 4}{literal}
                                'href="javascript:item_update(\''+data+'\');"'+
                                    {/literal}{/if}{literal}'';
                            
                            if(full.estado==="1"){
                                tipo="Descargar";
                                color="success";
                                href='href="javascript:item_download(\''+data+'\');"';
                            }
                            var boton = ''+
                                ...
                                '</a>'+
                                    {/literal}{/if}{literal}
                                '<a '+href+' class="m--font-'+color+' m-portlet__nav-link btn m-btn m-btn--hover-'+color+' m-btn--pill " title="View">'+
                                '<i class=" flaticon-file m--font-'+color+'"></i> <stroke></stroke>'+
                                '</a>'+
                                '';
                            return boton;
                        },
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            var boton = String(data).padStart(5,"0");
                            return boton;
                        },
                    },
                    {
