...
class Index extends Table {
    ...
    function obtenerTco(){
        ...
        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
                ...
                INNER JOIN usuario_rol ur2 on ur2.usuario_itemId=u2.itemId
                ,responsable_curtiembre t1
                INNER JOIN usuario_rol ur on ur.itemId=t1.usuario_itemId
            ...
            if($idItem!=''){
                if($tipoTabla=='tipo'){
                    ...
                    $sqlFrom = ' acta_cuero i
                             ...
                             ';
                    $sqlWhere = ' i.itemId='.$idItem.' and (ur.rol_itemId=1 or ur.rol_itemId=4) and u3.itemId='.$_SESSION["memberId"];
    ...
    function obtenerFilasReporte($id_acta=""){
        $longitudes=["0 a 1,14","1,15 a 1,19","1,20 a 1,24","1,25 a 1,29","1,30 a 1,34","1,35 a 1,39","1,40 a 1,49","1,50 a max"];
            ...
            $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
            $sql='SELECT  acl.*,CONCAT_WS(" a ",lc.min,lc.max) as longitud_literal,ac.estado
            ...
            if($info[0]["estado"]==2){
                for($i=0;$i<count($info);$i++){
                    $info[$i]["primera"]=empty($info[$i]["primera"])?$info[$i]["cantidad"]:$info[$i]["primera"];
                    $info[$i]["segunda"]=$info[$i]["segunda"]==null?0:$info[$i]["segunda"];
                    $info[$i]["rechazados"]=$info[$i]["rechazados"]==null?0:$info[$i]["rechazados"];
                    $info[$i]["pie_cuadrado"]=$info[$i]["pie_cuadrado"]==null?0:$info[$i]["pie_cuadrado"];
                    $info[$i]["precio_unidad"]=$info[$i]["precio_unidad"]==null?null:$info[$i]["precio_unidad"];
                    $info[$i]["precio_total"]=$info[$i]["precio_total"]==null?null:$info[$i]["precio_total"];
                }
            }
            $filas=$info;
    ...
    public function get_item_datatable_Rows($gestion=2019){
         ...
         */
        $ver=$this->verificar_permiso_usuarios(["1","4","6"]);
        if($ver){
            $tco=$this->obtenerTCO();
            if($tco["rol_itemId"]=="1"||$tco["rol_itemId"]=="6"){
                $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
            }
            else{
                $extraWhere .= " and responsable_curtiembre_itemId=".$tco["itemId"];
                $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
            }
            $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
    ...
    function item_delete($id){
        $campo_id="itemId";
        $res = $this->item_soft_delete_sbm($id,$campo_id,"acta_cuero");
        return $res;
    }
