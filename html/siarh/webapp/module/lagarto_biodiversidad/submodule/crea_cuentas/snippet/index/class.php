...
class Index extends Table {
    ...
    function get_item($idItem,$tipoTabla,$variante=""){
        ...
        if($idItem!=''){
            if($tipoTabla=='tipo'){
                $sqlSelect = ' i.*,ur.archivo archivo, t.itemId as tco_itemId,t.municipio_itemId,rcu.curtiembre_itemId,ur.rol_itemId,ur.ci_exp_itemId,ur.ci
                           , CONCAT_WS(" ",u1.nombre,u1.apellido) as userCreater
                           , CONCAT_WS(" ",u2.nombre,u2.apellido) as userUpdater';
                $sqlFrom = ' vrhr_snir.core_usuario i
                        LEFT JOIN usuario_rol ur on ur.usuario_itemId=i.itemId
                         ...
                         LEFT JOIN usuario_rol ur2 on ur2.usuario_itemId=u3.itemId
                         LEFT JOIN cazador c on c.usuario_itemId=ur.itemId
                         LEFT JOIN representante_legal rl on rl.usuario_itemId=ur.itemId
                         LEFT JOIN responsable_curtiembre rcu on rcu.usuario_itemId=ur.itemId
                         LEFT JOIN catalogo_tco t on (t.itemId=c.tco_itemId or t.itemId=rl.tco_itemId) ';
                $sqlWhere = ' ur.itemId='.$idItem.' and ur2.rol_itemId=1 and u3.itemId='.$_SESSION["memberId"];
                $sqlGroup = '';
            }else{
                $sqlSelect = ' i.*';
                $sqlFrom = ' usuario i,usuario u2';
                $sqlWhere = ' ur.itemId='.$idItem.' and u2.rol_itemId=1 and u2.itemId='.$_SESSION["memberId"];
            }
    ...
    function item_delete($id){
        $campo_id="itemId";
        $res = $this->item_soft_delete_sbm($id,$campo_id,"usuario_rol");
        return $res;
    }
