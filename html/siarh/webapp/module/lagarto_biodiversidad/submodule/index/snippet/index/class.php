...
class Index extends Table {
    ...
    function get_menu_principal2(){
        ...
        $sql = "select c.itemId,c.nombre,c.descripcion, c2.nombre as padre_nombre,c.padre ,c.class
                ...
                where c.activo=1 and c.padre is not NULL order by c2.orden, c.orden";
        $item = $db->Execute($sql);
        $item = $item->getRows();
    ...
    function get_menu_table($tabla){
        return "mmaya_biodiversidad_trazabilidad.".$tabla;
    }
    function core_sem($table){
        return "vrhr_snir.core_".$table;
    }
    function get_menu_principal(){
        ...
        global $db, $usuarioInfo;
          ...
          */
         $sql = "select * from ".$this->core_sem("submodulo")." as sm where sm.moduloId = '".$id."' and sm.parent=0 and sm.principal=0 and sm.activo=1 order by sm.orden";
         ...
         foreach ($modulo as $row){
             ...
             if($_SESSION["userv"]["tipoUsuario"]==0 || $_SESSION["userv"]["tipoUsuario"]==1){
                 $sql = "select m.carpeta as modulo_id_tipo_carpeta
                    ...
                    from ".$this->core_sem("submodulo")." as sm 
                    
                    left join ".$this->core_sem("modulo")."  as m on m.itemId = sm.modulo_id_tipo
                    left join ".$this->core_sem("submodulo")."  as sb on sb.itemId = sm.submodulo_id
                    left join ".$this->core_sem("modulo")." as m2 on m2.itemId = sb.moduloId 
                 ...
                 $sql = "select m.carpeta as modulo_id_tipo_carpeta
                    ...
                    join ".$this->core_sem("usuario_permisos")." as up on up.subModuloId = sm.itemId
    ...
    private function get_persona_info($id){
        global $db;
        $sql = "select * from vrhr_snir.core_usuario as u where u.itemId = '".$id."'";
        $item = $db->Execute($sql);
        $item = $item->fields;
        return $item;
    }
    ...
    public function getPhoto($type){
        ...
        $item = $this->get_persona_info($this->userv["itemId"]);
