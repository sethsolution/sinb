...
class Index extends Table {
    ...
    function obtenerDisponibleCarne(){
        $q1="SELECT SUM(tcc.total_carne) as carneT,SUM(tcc.total_charque) as charqueT
            FROM tco_custodia_carne tcc
            INNER JOIN representante_legal as rl1 on
            rl1.itemId=tcc.representante_legal_itemId
            INNER JOIN usuario_rol ur on ur.itemId=rl1.usuario_itemId
            INNER JOIN vrhr_snir.core_usuario usu1 on
            usu1.itemId=ur.usuario_itemId
            WHERE
            usu1.itemId=".$_SESSION["memberId"]."
	    and tcc.gestion_itemId='".date("Y")."'";
        $q2="SELECT SUM(ac.entregado_carne_primera+ac.entregado_carne_segunda) as carneE,SUM(ac.entregado_charque) as charqueE
            FROM acta_carne ac
            INNER JOIN representante_legal as rl3 on
            rl3.itemId=ac.representante_legal_itemId
            INNER JOIN usuario_rol ur2 on ur2.itemId=rl3.usuario_itemId
            INNER JOIN vrhr_snir.core_usuario usu2 on
            usu2.itemId=ur2.usuario_itemId
            WHERE
            ac.estado!=0 and
            usu2.itemId=".$_SESSION["memberId"]."
	    and ac.gestion_itemId='".date("Y")."'";
            
        $sumT = $this->dbm->Execute($q1);
        $sumT = $sumT->fields;
        $enT = $this->dbm->Execute($q2);
        $enT = $enT->fields;
        return [$sumT["carneT"]-$enT["carneE"],$sumT["charqueT"]-$enT["charqueE"]];
    }
    function obtenerTco(){

        $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
                $sql="SELECT  t1.itemId,ur2.rol_itemId
                ...
                WHERE u2.itemId= ".$_SESSION["memberId"]." and (u.itemId=".$_SESSION["memberId"]." or ur2.rol_itemId=1 or ur2.rol_itemId=4)
    ...
    function get_item($idItem,$tipoTabla,$variante=""){
        ...
        if($idItem!=''){
            if($tipoTabla=='tipo'){
                ...
                $sqlFrom = ' acta_carne i
                         LEFT JOIN vrhr_snir.core_usuario u1 on u1.itemId=i.userCreate
                         LEFT JOIN vrhr_snir.core_usuario u3 on u1.itemId=i.userCreate
                         LEFT JOIN vrhr_snir.core_usuario u2 on u2.itemId=i.userUpdate
                         LEFT JOIN usuario_rol ur on ur.usuario_itemId=u3.itemId
                         ';
                $sqlWhere = ' i.itemId='.$idItem.' and (ur.rol_itemId=1 or ur.rol_itemId=3) and u3.itemId='.$_SESSION["memberId"];
                $sqlGroup = ' GROUP BY i.itemId';
            }else{
                $sqlSelect = ' i.*';
                $sqlFrom = ' acta_cuero i ';
    ...
    function obtenerFilasReporte($id_acta=""){
        $longitudes=["0 a 1,14","1,15 a 1,19","1,20 a 1,24","1,25 a 1,29","1,30 a 1,34","1,35 a 1,39","1,40 a 1,49","1,50 a max"];
        if($id_acta==""){
        $filas=[];
        for($i=1;$i<=8;$i++){
            $filas[]=array(
            "longitud_literal"=>$longitudes[$i-1],    
            "longitud_cuero_itemId"=>$i,
            "cantidad"=>0,
            "primera"=>0,
            "segunda"=>0,
            "rechazados"=>0,
            ...
            $this->dbm->SetFetchMode(ADODB_FETCH_ASSOC);
            $sql='SELECT  acl.*,CONCAT_WS(" a ",lc.min,lc.max) as longitud_literal
            FROM  acta_cuero_longitud acl
            INNER JOIN longitud_cuero lc on 
            lc.itemId=acl.longitud_cuero_itemId
            WHERE acl.acta_cuero_itemId='.$id_acta.'
            ORDER BY acl.longitud_cuero_itemId'
            ;
    ...
    public function get_item_datatable_Rows($gestion=2019){
         ...
         */
        $extraWhere = "i.estado!=0 and i.gestion_itemId=".$gestion;
        $groupBy = "";
        $having = "";
        /**
         * Resultado de la consulta enviada
         */
        $ver=$this->verificar_permiso_usuarios(["1","3"]);
        if($ver){
            $tco=$this->obtenerTCO();
            if($tco){
                if($tco["rol_itemId"]=="1"){
                    ...
                    $extraWhere .= " and representante_legal_itemId=".$tco["itemId"];
                    $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
                }
                $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
                }   
    ...
    function item_delete($id){
        $campo_id="itemId";
        $res = $this->item_soft_delete_sbm($id,$campo_id,"acta_carne","estado=2");
        return $res;
    }
