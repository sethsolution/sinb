...
class Index extends Table {
    ...
    function get_item($idItem,$tipoTabla,$variante=""){
        ...
        if($idItem!=''){
            if($tipoTabla=='tipo'){
                $sqlSelect = ' i.*,t.tco,mn.nombre municipio,pv.nombre provincia,dp.nombre departamento
                           , CONCAT_WS(" ",u1.nombre,u1.apellido) as userCreater
                           , CONCAT_WS(" ",u2.nombre,u2.apellido) as userUpdater';
                $sqlFrom = ' gestion_tco i
                         ...
                         LEFT JOIN vrhr_snir.core_usuario u3 on u2.itemId=i.userUpdate
                         LEFT JOIN usuario_rol ur on u3.itemId=ur.usuario_itemId
                         LEFT JOIN catalogo_tco t on t.itemId=i.tco_itemId
                         LEFT JOIN vrhr_territorio.municipio mn on mn.itemId=t.municipio_itemId
                         LEFT JOIN vrhr_territorio.provincia pv on pv.itemId=mn.provinciaId
                         LEFT JOIN vrhr_territorio.departamento dp on dp.itemId=pv.departamentoId
                         ';
                $sqlWhere = ' i.itemId='.$idItem.' and ur.rol_itemId=1 and u3.itemId='.$_SESSION["memberId"];
                $sqlGroup = ' GROUP BY i.itemId';
            }else{
                $sqlSelect = ' i.*';
                $sqlFrom = ' gestion_tco i,usuario u2';
                $sqlWhere = ' i.itemId='.$idItem.' and u2.rol_itemId=1 and u2.itemId='.$_SESSION["memberId"];
            }
    ...
    public function get_item_datatable_Rows($gestion_itemId=2019){
         ...
         */
        $extraWhere = "i.estado=1 and i.gestion_itemId=".$gestion_itemId;
        $groupBy = "";
        $having = "";
        /**
        ...
        if($ver){
            ...
            if($tco){
                ...
                $extraWhere .= " and tco_itemId=".$tco["tco_itemId"];
                    $resultado = $this->get_grilla_datatable_simple($db,$grilla,$table, $primaryKey, $extraWhere, $groupBy, $having);
            }
            }
        }
    ...
    function item_delete($id){
        $campo_id="itemId";
        $res = $this->item_soft_delete_sbm($id,$campo_id,"gestion_tco");
        return $res;
    }
