...
class Snippet extends Table {
    ...
    function item_update($rec, $itemId, $que_form, $accion, $item_id, $archivo_adjunto) {
        $tabla = $this->tabla["cultivo_adjuntos"];
        /**
        ...
        if( $res["res"]==1){
            ...
            $adjunto = $this->item_adjunto_sbm($archivo_adjunto, $item_id, $res["id"], $item, $tabla, $accion, 'cult_itemid');
        }
        return $res;
    }
    ...
    public function get_item_datatable_Rows($id){
         ...
         */
        $table = $this->tabla["cultivo_adjuntos"];
        /**
         ...
         */
        $grilla = "grilla_cultivo_adjuntos";
        /**
    ...
    function item_delete($id, $campo_referencia, $tabla_referencia) {
        ...
        $dir = $this->get_dir_item_archivo_sbm($item["cult_itemid"], 0);
    ...
    /**
     * Función actualiza el campo adj_img_itemid de la tabla cultivos
     **/
    function set_recurso_imagen($id, $adjId) {
        ...
        $sql = "UPDATE cultivo_adjuntos SET estado=NULL WHERE estado='' OR estado='foto' AND cult_itemid=".$id;
        $datos = $dbm->Execute($sql);
        $sql = "UPDATE cultivo_adjuntos SET estado='foto' WHERE itemId=".$adjId;
        $datos = $dbm->Execute($sql);
        $dbm->Close();
        if ($datos) {
            $respuesta = array(
                'res' => 1,
                'msg' => 'Este recurso se estableció como imagen por defecto del cultivo.'
            );
        } else {
            $respuesta = array(
                'res' => 2,
