...
var snippet_general_form = function() {
    ...
    var cboDeptoId = $('#cboDeptoId');
    var cboMunicipioId = $('#cboMunicipioId');
    var itemsDepto = '{/literal}{$item.depto_itemid}{literal}'.split(",");
    ...
    var getDepartamentos = function () {
        ...
        cboDeptoId.empty();
        if(macroId!="") {
            $.post("{/literal}{$getModule}{literal}&accion=general_getDeptos"
                , {macroId: macroId}
                , function (respuesta, textStatus, jqXHR) {
                    //selOption = $('<option></option>');
                    //cboDeptoId.append(selOption.attr("value", "").text("-- Seleccione una opción --"));
                    for (var clave in respuesta) {
                        cboDeptoId.append($('<option></option>').attr("value", respuesta[clave].itemId).text(respuesta[clave].nombre));
                    }
                    if (type == 'update') {
                        cboDeptoId.val(itemsDepto).trigger('change');
                    }
                }
                , 'json');
        } else {
            cboDeptoId.append($('<option></option>').attr("value", "").text("--Seleccione una opción--"));
        }
    };

    var getMunicipios = function () {
        var deptoId = cboDeptoId.val();
