...
switch($accion) {
    ...
    case 'itemUpdate':
        ...
        $menu_tab = $subObjItem->get_item_tab_sbm($type, "tab_proyecto_metas");

        $smarty->assign("menu_tab", $menu_tab);
        $smarty->assign("menu_tab_active", "general");

        if($type=="update"){
            $item = $subObjItem->get_item($id,"c_proyecto_metas");
    ...
    case 'itemupdatesql':
        $respuesta = $subObjItem->item_update($item, $itemId, "campos_proyecto_metas", $type);
        $core->print_json($respuesta);
        break;
