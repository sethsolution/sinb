...
<script>
    ...
    var btnVerResumen = $('#btnVerResumen');

    function item_update(id) {
        var url = '{/literal}{$getModule}{literal}&accion=metas_getForm&id={/literal}{$id}{literal}&proymetaId='+id+'&type=update';
    ...
    function itemDeleteAction(id) {
        ...
        $.get( "{/literal}{$getModule}{literal}",
            {accion:"metas_itemDelete", random:randomnumber, id:id},
            function(res){
                if(res.res == 1){
    ...
    function cargarFormulario() {
        var url = '{/literal}{$getModule}{literal}&accion=metas_getForm&id={/literal}{$id}{literal}&type=new';
        $.get(url, function(respuesta) {
            modalContent.html('');
            modalContent.html(respuesta);
            modalWindow.modal('show');
    ...
    function mostrarResumen() {
        var url = '{/literal}{$getModule}{literal}&accion=metas_verResumen&id={/literal}{$id}{literal}';
        $.get(url, function(respuesta) {
            modalContent.html('');
            modalContent.html(respuesta);
            modalWindow.modal('show');
    ...
    var snippet_datatable_list = function() {
        ...
        var exporta_titulo = "LISTADO DE METAS DE PROYECTO";
        ...
        var initTable1 = function() {
            ...
            table_list = $('#index_lista').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{/literal}{$getModule}{literal}&accion=metas_getItemList&id={/literal}{$id}{literal}',
                    type: 'POST',
                    data: {},
                },
                columns: [
                ...
                columnDefs: [
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var boton = '{/literal}{if $privFace.editar == 1}{literal}' + item_button_update(full.itemId) + '{/literal}{/if}{literal}{/literal}{if $privFace.eliminar == 1}{literal}' + item_button_delete(full.itemId) + '{/literal}{/if}{literal}';
                            return boton;
                        }
                    },
                    {
    ...
    jQuery(document).ready(function() {
        ...
        btnVerResumen.on('click', function() {
            if (table_list.data().count() != 0) {
                mostrarResumen();
            } else {
                swal('Advertencia!', 'El proyecto no contiene metas.', 'warning');
            }
